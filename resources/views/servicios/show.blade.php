...
<div class="container-fluid max-w-7xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white uppercase tracking-wider">Detalle de Servicio</h1>
        ...
        <div class="flex gap-2">
            ...
            @can('servicios.edit')
                <a href="{{ route('servicios.edit', $servicio) }}{{ request()->boolean('wizard') ? '?wizard=1' : '' }}"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <p class="text-lg font-bold text-gray-900 dark:text-white">{{ $servicio->id }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Nombre</label>
                        <p class="text-md font-medium text-gray-900 dark:text-white">{{ $servicio->nombre }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Descripción</label>
                        <p class="text-md font-medium text-gray-900 dark:text-white">{{ $servicio->descripcion ?? 'N/A' }}</p>
                    </div>
                    <div>
                        ...
                        <span @class([
                            ...
                            'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' => $servicio->estado === 'activo',
                            'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200' => $servicio->estado === 'inactivo',
                            'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200' => !in_array($servicio->estado, ['activo','inactivo']),
                        ])>
                    ...
                    <div>
                        ...
                        <p class="text-md font-medium text-gray-900 dark:text-white">{{ $servicio->proyecto->nombre ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Tiene Medidor</label>
                        <span @class([
                            'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium',
                            'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200' => $servicio->tiene_medidor,
                            'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400' => !$servicio->tiene_medidor,
                        ])>
                    ...
                    @if($servicio->tiene_medidor)
                        <div>
                            <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Unidad de Medida</label>
                            <p class="text-md font-medium text-gray-900 dark:text-white">{{ $servicio->unidad_medida ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Precio por Unidad</label>
                            <p class="text-md font-medium text-gray-900 dark:text-white">L {{ number_format($servicio->precio_por_unidad_de_medida, 2) }}</p>
                        </div>
                    @endif
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Es Aportación</label>
                        <span @class([
                            'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium',
                            'bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200' => $servicio->es_aportacion,
                            'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400' => !$servicio->es_aportacion,
                        ])>
                ...
                <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/30 border-t border-gray-200 dark:border-gray-700">
                    <div class="text-[10px] text-gray-400 flex flex-col gap-1">
                        <span>Creado: {{ $servicio->created_at->format('d/m/Y H:i') }}</span>
                ...
                <div class="p-6">
                    <div class="flow-root">
                        <ul role="list" class="-mb-8">
                            @forelse($servicio->auditLogs as $log)
                                <li>
