...
@section('title', 'Detalle de Aportación')
...
<div class="container-fluid max-w-7xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white uppercase tracking-wider">Detalle de Aportación</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Viendo la información detallada y el historial de cambios de la aportación.</p>
        </div>
        <div class="flex gap-2">
            ...
            @can('aportacion.edit')
                <a href="{{ route('aportacion.edit', $aportacion) }}{{ request()->boolean('wizard') ? '?wizard=1' : '' }}"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <p class="text-lg font-bold text-gray-900 dark:text-white">{{ $aportacion->id_aportacion }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Miembro</label>
                        <p class="text-md font-medium text-gray-900 dark:text-white">{{ $aportacion->miembro->nombre ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Proyecto</label>
                        <p class="text-md font-medium text-gray-900 dark:text-white">{{ $aportacion->proyecto->nombre ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Monto</label>
                        <p class="text-md font-medium text-gray-900 dark:text-white">
                            L {{ number_format($aportacion->monto, 2) }}
                        </p>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Fecha de Aportación</label>
                        <p class="text-md font-medium text-gray-900 dark:text-white">
                            {{ $aportacion->fecha_aportacion->format('d/m/Y') }}
                        </p>
                    </div>
                    <div>
                ...
                <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/30 border-t border-gray-200 dark:border-gray-700">
                    <div class="text-[10px] text-gray-400 flex flex-col gap-1">
                        <span>Creado: {{ $aportacion->created_at->format('d/m/Y H:i') }}</span>
                        <span>Actualizado: {{ $aportacion->updated_at->format('d/m/Y H:i') }}</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="p-6">
                    <div class="flow-root">
                        <ul role="list" class="-mb-8">
                            @forelse($aportacion->auditLogs as $log)
                                <li>
