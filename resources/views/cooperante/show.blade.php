...
@section('title', 'Detalle de Cooperante')
...
<div class="container-fluid max-w-7xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white uppercase tracking-wider">Detalle de Cooperante</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Viendo la información detallada y el historial de actividad.</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('cooperantes.index') }}" class="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors duration-200 text-sm font-medium">
                Volver
            </a>
            @can('cooperante.edit')
                <a href="{{ route('cooperantes.edit', $cooperante) }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-200 text-sm font-medium">
                    Editar
                </a>
            @endcan
        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <p class="text-lg font-bold text-gray-900 dark:text-white">{{ $cooperante->id_cooperante }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-md font-medium text-gray-900 dark:text-white">{{ $cooperante->nombre }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Tipo de Cooperante</label>
                        <p class="text-md font-medium text-gray-900 dark:text-white">{{ $cooperante->tipo_cooperante }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Teléfono</label>
                        <p class="text-md font-medium text-gray-900 dark:text-white">{{ $cooperante->telefono }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Dirección</label>
                        <p class="text-md font-medium text-gray-900 dark:text-white">{{ $cooperante->direccion }}</p>
                    </div>
                </div>
                <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/30 border-t border-gray-200 dark:border-gray-700">
                    <div class="text-[10px] text-gray-400 flex flex-col gap-1">
                        <span>Creado: {{ $cooperante->created_at->format('d/m/Y H:i') }}</span>
                        <span>Actualizado: {{ $cooperante->updated_at->format('d/m/Y H:i') }}</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="p-6">
                    <div class="flow-root">
                        <ul role="list" class="-mb-8">
                            @forelse($cooperante->auditLogs as $log)
                                <li>
