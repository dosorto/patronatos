<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-300 mt-1">Administra el catálogo de servicios y sus tarifas</p>
        </div>
            ...
            <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-b border-gray-200 dark:border-gray-600">
                <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                    <div class="flex items-center gap-4">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Lista de Servicios</h2>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                            {{ $servicios->total() }} {{ $servicios->total() === 1 ? 'registro' : 'registros' }}
                        </span>
                    </div>
                    ...
                    <div class="flex items-center gap-3 flex-1 max-w-lg w-full">
                        <div class="relative flex-1">
                            ...
                            <input
                                ...
                                placeholder="Buscar por nombre, estado o proyecto..."
                            >
                        </div>
                        <select
                    ...
                    <thead class="bg-gray-100 dark:bg-gray-900">
                        <tr>
                            <th class="px-6 py-3 text-left">
                                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Servicio</span>
                            </th>
                            <th class="px-6 py-3 text-left">
                                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Precio</span>
                            </th>
                            <th class="px-6 py-3 text-left">
                        ...
                        @forelse($servicios as $servicio)
                            ...
                            >
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center mr-3 shadow-sm group-hover:scale-110 transition-transform">
                                            <span class="text-white text-xs font-bold">{{ strtoupper(substr($servicio->nombre, 0, 2)) }}</span>
                                        </div>
                                        <div>
                                            <div class="font-medium text-gray-900 dark:text-white">{{ $servicio->nombre }}</div>
                                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ Str::limit($servicio->descripcion, 40) }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    ...
                                    @if($servicio->tiene_medidor)
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                            L {{ number_format($servicio->precio_por_unidad_de_medida, 2) }} / {{ $servicio->unidad_medida }}
                                        </div>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span class="text-sm text-gray-400 dark:text-gray-500">Sin medidor</span>
                                    @endif
                                    @if($servicio->es_aportacion)
                                        <span class="ml-1 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200">
                                            Aportación
                                        </span>
                                    @endif
                                </td>
                                <td class="px-6 py-4" onclick="event.stopPropagation()">
                                    <div class="flex items-center space-x-2">
                                        @can('servicios.view')
                                            <a href="{{ route('servicios.show', $servicio->id) }}{{ request()->has('wizard') ? '?wizard=1' : '' }}"
                                        ...
                                        @can('servicios.edit')
                                            <a href="{{ route('servicios.edit', $servicio->id) }}{{ request()->has('wizard') ? '?wizard=1' : '' }}"
                                        ...
                                        @can('servicios.delete')
                                            <button
                                                wire:click="confirmServicioDeletion('{{ $servicio->id }}', '{{ $servicio->nombre }}')"
                                ...
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 dark:text-gray-400 mb-4 max-w-sm">
                                            ...
                                            @else
                                                Comienza agregando servicios para gestionar el catálogo.
                                            @endif
                                        </p>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full overflow-hidden border border-gray-200 dark:border-gray-700 anim-scale-in">
                <div class="p-6">
                    ...
                    <div class="text-center">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">¿Eliminar Servicio?</h3>
                        <p class="text-gray-600 dark:text-gray-400">
                            Estás a punto de eliminar el servicio <span class="font-semibold text-gray-900 dark:text-white">{{ $servicioNameBeingDeleted }}</span>.
