<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-300 mt-1">Administra el registro de empleados y sus datos básicos</p>
        </div>
        ...
        <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-b border-gray-200 dark:border-gray-600">
            <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                <div class="flex items-center gap-4">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Lista de Empleados</h2>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                        {{ $empleados->total() }} {{ $empleados->total() === 1 ? 'registro' : 'registros' }}
                    </span>
                </div>
                ...
                <div class="flex items-center gap-3 flex-1 max-w-lg w-full">
                    <div class="relative flex-1">
                        ...
                        <input
                            ...
                            placeholder="Buscar por persona, cargo u organización..."
                        >
                    </div>
            ...
            <table class="w-full">
                <thead class="bg-gray-100 dark:bg-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left">
                            <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Sueldo Mensual</span>
                        </th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($empleados as $empleado)
                        <tr
                            ...
                            onclick="window.location='{{ route('empleado.show', $empleado) }}'"
                        >
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center mr-3 shadow-sm group-hover:scale-110 transition-transform">
                                        <span class="text-white text-xs font-bold">
                                            {{ strtoupper(substr($empleado->persona->nombre ?? '?', 0, 1) . substr($empleado->persona->apellido ?? '?', 0, 1)) }}
                                        </span>
                                    </div>
                                    <div>
                                        <div class="font-medium text-gray-900 dark:text-white">
                                            {{ $empleado->persona->nombre_completo ?? 'N/A' }}
                                        </div>
                            ...
                            <td class="px-6 py-4" onclick="event.stopPropagation()">
                                <div class="flex items-center space-x-2">
                                    ...
                                    @can('empleado.delete')
                                        <button
                                            wire:click="confirmEmpleadoDeletion({{ $empleado->id }}, '{{ $empleado->persona->nombre_completo ?? $empleado->id }}')"
                            ...
                            <td colspan="5" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 dark:text-gray-400 mb-4 max-w-sm">
                                        ...
                                        @else
                                            Comienza agregando los empleados para gestionar la base de datos de manera organizada.
                                        @endif
                                    </p>
                                    @if(!$search)
                                        @can('empleado.create')
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full overflow-hidden border border-gray-200 dark:border-gray-700 anim-scale-in">
                <div class="p-6">
                    ...
                    <div class="text-center">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">¿Eliminar Empleado?</h3>
                        <p class="text-gray-600 dark:text-gray-400">
                            Estás a punto de eliminar a <span class="font-semibold text-gray-900 dark:text-white">{{ $empleadoNameBeingDeleted }}</span>.
