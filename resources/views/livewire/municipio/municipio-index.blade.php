                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($municipios as $municipio)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            <td class="px-6 py-4">
                                <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $municipio->nombre }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-sm text-gray-500 dark:text-gray-400">{{ $municipio->departamento->nombre ?? 'N/A' }}</span>
                            </td>

                            <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $municipio->id }}</td>
                            <td class="px-6 py-4 flex gap-2">
                                @can('municipio.view')
                                    <a href="{{ route('municipio.show', $municipio) }}" class="text-gray-600 hover:text-gray-800" title="Ver">👁️</a>
                                @endcan
                                @can('municipio.edit')
                                    <a href="{{ route('municipio.edit', $municipio) }}" class="text-blue-600 hover:text-blue-800" title="Editar">✏️</a>
                                @endcan
                                @can('municipio.delete')
                                    <button wire:click="confirmMunicipioDeletion({{ $municipio->id }}, '{{ $municipio->nombre }}')" 
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900 bg-opacity-50">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full p-6">
                ...
                <p class="text-gray-600 dark:text-gray-300 mb-6">
                    ¿Estás seguro de que deseas eliminar el municipio <strong>{{ $municipioNameBeingDeleted }}</strong>? Esta acción no se puede deshacer.
                </p>
