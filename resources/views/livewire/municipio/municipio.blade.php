<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Gestión de Municipios</h1>
            <p class="text-gray-600 dark:text-gray-300 mt-1">Administra el registro de los municipios y sus datos básicos</p>
        </div>

        <div class="flex flex-wrap gap-2">
            @can('municipios.export')
                <button 
                    wire:click="export"
            ...
            <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-b border-gray-200 dark:border-gray-600">
                <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                    <div class="flex items-center gap-4">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Lista de Municipios</h2>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                            {{ $municipios->total() }} {{ $municipios->total() === 1 ? 'registro' : 'registros' }}
                        </span>
                    </div>
                ...
                <table class="w-full">
                    <thead class="bg-gray-100 dark:bg-gray-900">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left">
                                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Código</span>
                            </th>
                            <th class="px-6 py-3 text-left">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($municipios as $municipio)
                            ...
                            >
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900 dark:text-white truncate max-w-xs">{{ $municipio->descripcion ?? 'N/A' }}</div>
                                </td>
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        @can('municipios.edit')
                                            <a href="{{ route('municipios.edit', $municipio) }}"
                                               class="text-yellow-600 hover:text-yellow-800 dark:text-yellow-400 dark:hover:text-yellow-500">
                                                Editar
                                            </a>
                                        @endcan
                                        ...
                                        @can('municipios.delete')
                                            <button 
                                                wire:click="confirmDelete({{ $municipio->id }})"
                                ...
                                <td colspan="4" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 dark:text-gray-400 mb-4 max-w-sm">
                                            Intenta ajustar los filtros o agrega un nuevo municipio para comenzar.
                                        </p>
                                        @if(!$search && auth()->user()->can('municipios.create'))
                                            <a href="{{ route('municipios.create') }}"
                                               class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors duration-200 flex items-center shadow-sm">
                                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full overflow-hidden border border-gray-200 dark:border-gray-700 anim-scale-in">
                <div class="p-6">
                    ...
                    <div class="text-center">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">¿Eliminar Municipio?</h3>
                        <p class="text-gray-600 dark:text-gray-400">
                            Estás a punto de eliminar a <span class="font-semibold text-gray-900 dark:text-white">{{ $municipioNameBeingDeleted }}</span>. 
