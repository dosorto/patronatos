<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-300 mt-1">Administra salarios, mantenimientos y otros pagos realizados</p>
        </div>
            ...
            <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-b border-gray-200 dark:border-gray-600">
                <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                    <div class="flex items-center gap-4">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Lista de Pagos</h2>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                            {{ $pagos->total() }} {{ $pagos->total() === 1 ? 'registro' : 'registros' }}
                        </span>
                    </div>
                    ...
                    <div class="flex items-center gap-3 flex-1 max-w-lg w-full">
                        <div class="relative flex-1">
                            ...
                            <input
                                ...
                                placeholder="Buscar por nombre, descripción, total o tipo..."
                            >
                        </div>
                    ...
                    <thead class="bg-gray-100 dark:bg-gray-900">
                        <tr>
                            <th class="px-6 py-3 text-left">
                                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Beneficiario</span>
                            </th>
                            <th class="px-6 py-3 text-left">
                                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tipo de Pago</span>
                            </th>
                            <th class="px-6 py-3 text-left">
                        ...
                        @forelse($pagos as $pago)
                            @php
                                $nombrePago = $pago->nombre_persona
                                    ?: (($pago->empleado?->persona?->nombre ?? '') . ' ' . ($pago->empleado?->persona?->apellido ?? ''));

                                $nombrePago = trim($nombrePago) !== '' ? trim($nombrePago) : 'Pago registrado';
                            ...
                            <tr 
                                ...
                                onclick="window.location='{{ $pago->recibo ? route('recibo.show', $pago->recibo) : route('pago.show', $pago) }}'"
                            ...
                            >
                                    ...
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center mr-3 shadow-sm group-hover:scale-110 transition-transform">
                                            <span class="text-white text-xs font-bold">
                                                {{ strtoupper(substr($nombrePago, 0, 2)) }}
                                            </span>
                                        </div>
                                        <div>
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-900 dark:text-white">
                                        {{ optional($pago->fecha_pago)->format('d/m/Y') ?? 'N/A' }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4" onclick="event.stopPropagation()">
                                    <div class="flex items-center space-x-2">
                                        @can('pago.view')
                                            @if($pago->recibo)
                                                <a href="{{ route('recibo.show', $pago->recibo) }}"
                                ...
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 dark:text-gray-400 mb-4 max-w-sm">
                                            ...
                                            @else
                                                Comienza agregando pagos para llevar un control organizado de egresos.
                                            @endif
                                        </p>
                                        @if(!$search && auth()->user()->can('pago.create'))
                                            <a href="{{ route('pago.create') }}"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full overflow-hidden border border-gray-200 dark:border-gray-700 anim-scale-in">
                <div class="p-6">
                    ...
                    <div class="text-center">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">¿Eliminar Pago?</h3>
                        <p class="text-gray-600 dark:text-gray-400">
                            Estás a punto de eliminar el pago <span class="font-semibold text-gray-900 dark:text-white">{{ $pagoNameBeingDeleted }}</span>.
