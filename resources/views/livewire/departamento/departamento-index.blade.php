            ...
            <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-b border-gray-200 dark:border-gray-600">
                <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                    <div class="flex items-center gap-4">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Lista de Departamentos</h2>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                            {{ $departamentos->total() }} {{ $departamentos->total() === 1 ? 'registro' : 'registros' }}
                        </span>
                    </div>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($departamentos as $departamento)
                            <tr 
                                ...
                                onclick="window.location='{{ route('departamento.show', $departamento) }}'"
                            >
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center mr-3 shadow-sm group-hover:scale-110 transition-transform">
                                            <span class="text-white text-xs font-bold">{{ strtoupper(substr($departamento->nombre, 0, 2)) }}</span>
                                        </div>
                                        <div>
                                            <div class="font-medium text-gray-900 dark:text-white">{{ $departamento->nombre }}</div>
                                ...
                                <td class="px-6 py-4" onclick="event.stopPropagation()">
                                    <div class="flex items-center space-x-2">
                                        @can('departamento.view')
                                            <a href="{{ route('departamento.show', $departamento) }}"
                                        ...
                                        @can('departamento.delete')
                                            <button
                                                wire:click="confirmDepartamentoDeletion('{{ $departamento->id }}', '{{ $departamento->nombre }}')"
                                ...
                                <td colspan="3" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 dark:text-gray-400 mb-4 max-w-sm">
                                            ...
                                            @else
                                                Comienza agregando los departamentos para gestionar la base de datos geográfica de manera organizada.
                                            @endif
                                        </p>
                                        @if(!$search && auth()->user()->can('departamento.create'))
                                            <a href="{{ route('departamento.create') }}"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full overflow-hidden border border-gray-200 dark:border-gray-700 anim-scale-in">
                <div class="p-6">
                    ...
                    <div class="text-center">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">¿Eliminar Departamento?</h3>
                        <p class="text-gray-600 dark:text-gray-400">
                            Estás a punto de eliminar el departamento <span class="font-semibold text-gray-900 dark:text-white">{{ $departamentoNameBeingDeleted }}</span>. 
