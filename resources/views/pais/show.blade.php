...
<div class="container-fluid max-w-7xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white uppercase tracking-wider">Detalle de País</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Viendo la información detallada del país.</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('pais.index') }}" class="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors duration-200 text-sm font-medium">
                Volver
            </a>
            @can('pais.edit')
                <a href="{{ route('pais.edit', $pais) }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-200 text-sm font-medium">
                    Editar
                </a>
            @endcan
        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <p class="text-md font-medium text-gray-900 dark:text-white">{{ $pais->id }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-tighter block">Nombre del País</label>
                        <p class="text-md font-medium text-gray-900 dark:text-white">{{ $pais->nombre }}</p>
                    </div>
                    <div>
                        ...
                        <p class="text-md font-medium text-gray-900 dark:text-white">{{ $pais->created_at->format('d/m/Y H:i') }}</p>
                    </div>
                    <div>
                        ...
                        <p class="text-md font-medium text-gray-900 dark:text-white">{{ $pais->updated_at->format('d/m/Y H:i') }}</p>
                    </div>
                </div>
            </div>
        </div>
                                            ...
                                            <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                                <div>
                                                    <p class="text-sm text-gray-500 dark:text-gray-400">
                                                        <span class="font-bold text-gray-900 dark:text-white uppercase text-xs">{{ ucfirst($log->event) }}</span> por 
