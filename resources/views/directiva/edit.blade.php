...
@section('title', 'Editar Miembro Directiva')

@section('content')
<div class="container-fluid max-w-4xl">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Miembro Directiva</h1>
        <p class="text-gray-600 dark:text-gray-300 mt-1">Actualiza los datos del cargo directivo</p>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        <form action="{{ route('directiva.update', $directiva) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="miembro_id" id="miembro_id" required
                        ...
                        @foreach($miembros as $miembro)
                            <option value="{{ $miembro->id }}" data-dni="{{ $miembro->persona->dni }}" {{ old('miembro_id', $directiva->miembro_id) == $miembro->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="organizacion_id" id="organizacion_id" required
                        ...
                        @foreach($organizaciones as $organizacion)
                            <option value="{{ $organizacion->id_organizacion }}" {{ old('organizacion_id', $directiva->organizacion_id) == $organizacion->id_organizacion ? 'selected' : '' }}>
                                {{ $organizacion->nombre }}
                            </option>
                        @endforeach
                    </select>
