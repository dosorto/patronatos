...
<div class="container-fluid max-w-4xl">
    <div class="mb-6">
        ...
        <p class="text-gray-600 dark:text-gray-300 mt-1">Modifica los datos de la organización</p>
    </div>
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        <form action="{{ route('organizacion.update', $organizacion->id_organizacion) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="nombre" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Nombre *</label>
                    <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $organizacion->nombre) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="rtn" id="rtn" value="{{ old('rtn', $organizacion->rtn) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="telefono" id="telefono" value="{{ old('telefono', $organizacion->telefono) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="direccion" id="direccion" value="{{ old('direccion', $organizacion->direccion) }}" required
                ...
                <div>
                    ...
                    <select name="id_tipo_organizacion" id="id_tipo_organizacion" required
                        ...
                        @foreach($tiposOrganizacion as $tipo)
                            <option value="{{ $tipo->id_tipo_organizacion }}" {{ old('id_tipo_organizacion', $organizacion->id_tipo_organizacion) == $tipo->id_tipo_organizacion ? 'selected' : '' }}>
                                {{ $tipo->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="id_departamento" id="id_departamento" required
                        ...
                        @foreach($departamentos as $departamento)
                            <option value="{{ $departamento->id }}" {{ old('id_departamento', $organizacion->id_departamento) == $departamento->id ? 'selected' : '' }}>
                                {{ $departamento->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="id_municipio" id="id_municipio" required
                        ...
                        @foreach($municipios as $municipio)
                            <option value="{{ $municipio->id }}" {{ old('id_municipio', $organizacion->id_municipio) == $municipio->id ? 'selected' : '' }}>
                                {{ $municipio->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="fecha_creacion" id="fecha_creacion" value="{{ old('fecha_creacion', $organizacion->fecha_creacion?->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <select name="estado" id="estado" required
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
                        <option value="Activo" {{ old('estado', $organizacion->estado) == 'Activo' ? 'selected' : '' }}>Activo</option>
                        <option value="Inactivo" {{ old('estado', $organizacion->estado) == 'Inactivo' ? 'selected' : '' }}>Inactivo</option>
                    </select>
