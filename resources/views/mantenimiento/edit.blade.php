...
<div class="container-fluid max-w-4xl">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Mantenimiento</h1>
        <p class="text-gray-600 dark:text-gray-300 mt-1">Actualiza los datos del mantenimiento</p>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="mb-4">
                    ...
                    <select name="tipo_mantenimiento" id="tipo_mantenimiento" required
                        ...
                        <option value="General" {{ old('tipo_mantenimiento', $mantenimiento->tipo_mantenimiento) == 'General' ? 'selected' : '' }}>General</option>
                ...
                <div class="mb-4">
                    ...
                    <select name="activo_id" id="activo_id"
                        ...
                        @foreach($activos as $activo)
                            <option value="{{ $activo->id }}" {{ old('activo_id', $mantenimiento->activo_id) == $activo->id ? 'selected' : '' }}>
                                {{ $activo->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="fecha_registro" id="fecha_registro" 
                        value="{{ old('fecha_registro', $mantenimiento->fecha_registro?->format('Y-m-d')) }}" required
                ...
                <div class="mb-4">
                    ...
                    <select name="prioridad" id="prioridad" required
                        ...
                        <option value="Baja" {{ old('prioridad', $mantenimiento->prioridad) == 'Baja' ? 'selected' : '' }}>Baja</option>
                        <option value="Media" {{ old('prioridad', $mantenimiento->prioridad) == 'Media' ? 'selected' : '' }}>Media</option>
                        <option value="Alta" {{ old('prioridad', $mantenimiento->prioridad) == 'Alta' ? 'selected' : '' }}>Alta</option>
                    </select>
                    @error('prioridad')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" name="costo_estimado" id="costo_estimado" 
                            value="{{ old('costo_estimado', $mantenimiento->costo_estimado) }}"
                ...
                <div class="mb-4 md:col-span-2">
                    ...
                    <textarea name="descripcion" id="descripcion" rows="3" required
                        ...
                        placeholder="Describe el trabajo realizado o por realizar">{{ old('descripcion', $mantenimiento->descripcion) }}</textarea>
                    @error('descripcion')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
